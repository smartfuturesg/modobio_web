...
class PowerAttemptsPushPull(Schema):
    weight = fields.Integer(
        metadata={"description": "weight of exercise in PSI"},
        validate=validate.Range(min=0, max=60),
        missing=None,
    )
    attempt_1 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=4000),
        missing=None,
    )
    attempt_2 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=4000),
        missing=None,
    )
    attempt_3 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=4000),
        missing=None,
    )
...
class PowerPushPull(Schema):
    left = fields.Nested(
        PowerAttemptsPushPull, missing=PowerAttemptsPushPull().load({})
    )
    right = fields.Nested(
        PowerAttemptsPushPull, missing=PowerAttemptsPushPull().load({})
    )
...
class PowerLegPress(Schema):
    left = fields.Nested(
        PowerAttemptsLegPress, missing=PowerAttemptsLegPress().load({})
    )
    right = fields.Nested(
        PowerAttemptsLegPress, missing=PowerAttemptsLegPress().load({})
    )
    bilateral = fields.Nested(
        PowerAttemptsLegPress, missing=PowerAttemptsLegPress().load({})
    )
...
class PowerAssessmentSchema(Schema):
    ...
    upper_watts_per_kg = fields.Float(
        metadata={"description": "watts per kg upper body"},
        validate=validate.Range(min=0, max=120),
        missing=None,
    )
    lower_watts_per_kg = fields.Float(
        metadata={"description": "watts per kg lower body"},
        validate=validate.Range(min=0, max=300),
        missing=None,
    )
    vital_weight = fields.Float(
        metadata={"description": "weight pulled from doctor physical data"},
        dump_only=True,
    )
    ...
    def ravel(self, data, **kwargs):
        nested = {
            ...
            "push_pull": {
                "left": {
                    "weight": data.keiser_upper_l_weight,
        ...
        recent_physical = (
            MedicalPhysicalExam.query.filter_by(user_id=data.user_id)
...
class StrengthAttemptsPushPull(Schema):
    weight = fields.Integer(
        metadata={"description": "weight of exercise in PSI"},
        validate=validate.Range(min=0, max=350),
        missing=None,
    )
    attempt_1 = fields.Integer(
        metadata={"description": ""},
...
class StrengthPushPull(Schema):
    ...
    left = fields.Nested(
        StrengthAttemptsPushPull, missing=StrengthAttemptsPushPull().load({})
    )
    right = fields.Nested(
        StrengthAttemptsPushPull, missing=StrengthAttemptsPushPull().load({})
    )
...
class StrenghtAssessmentSchema(Schema):
    ...
    def unravel(self, data, **kwargs):
        flat_data = {
            ...
            "upper_push_r_estimated_10rm": data["upper_push"]["right"][
                "estimated_10rm"
            ],
            ...
            "upper_push_bi_estimated_10rm": data["upper_push"]["bilateral"][
                "estimated_10rm"
            ],
            ...
            "upper_pull_r_estimated_10rm": data["upper_pull"]["right"][
                "estimated_10rm"
            ],
            ...
            "upper_pull_bi_estimated_10rm": data["upper_pull"]["bilateral"][
                "estimated_10rm"
            ],
        }
...
class ToeTouchTestSchema(Schema):
    ...
    depth = fields.String(missing=None)
    pelvis_movement = fields.List(
        fields.String,
        metadata={
            "description": (
                "Descriptors for this assessment must be in the following"
                f" picklist: {pelvis_movement_test_options}"
            )
        },
        missing=[None],
    )
    ribcage_movement = fields.List(
        fields.String,
        metadata={
            "description": (
                "Descriptors for this assessment must be in the following"
                f" picklist: {ribcage_movement_test_options}"
            )
        },
        missing=[None],
    )
        ...
        for option in value:
            if option not in self.ribcage_movement_test_options and option != None:
                raise ValidationError(
                    f"{option} is not a valid movement descriptor. Use one of"
                    f" the following {self.ribcage_movement_test_options}"
                )
    ...
    def valid_pelvis_movement(self, value):
        for option in value:
            if option not in self.pelvis_movement_test_options and option != None:
                raise ValidationError(
                    f"{option} is not a valid movement descriptor. Use one of"
                    f" the following {self.pelvis_movement_test_options}"
                )
...
class StandingRotationSchema(Schema):
    right = fields.Nested(
        StandingRotationNotesSchema,
        missing=StandingRotationNotesSchema().load({}),
    )
    left = fields.Nested(
        StandingRotationNotesSchema,
        missing=StandingRotationNotesSchema().load({}),
...
class MovementAssessmentSchema(Schema):
    ...
    standing_rotation = fields.Nested(
        StandingRotationSchema, missing=StandingRotationSchema().load({})
    )
    ...
    def ravel(self, data, **kwargs):
        nested = {
            ...
            "standing_rotation": {
                "right": {"notes": data.standing_rotation_r_notes},
                "left": {"notes": data.standing_rotation_l_notes},
            },
        }
...
class HeartAssessmentSchema(ma.SQLAlchemyAutoSchema):
    ...
    user_id = fields.Integer(missing=0)
    vital_heartrate = fields.Float(
        metadata={"description": "vital_heartrate pulled from doctor physical data"},
        dump_only=True,
    )
...
class MoxyAssessmentSchema(ma.SQLAlchemySchema):
    ...
    vl_side = fields.String(
        metadata={"description": "vl_side must be either right or left"},
        missing=None,
    )
    performance_baseline = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=100),
        missing=None,
    )
    recovery_baseline = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=100),
        missing=None,
    )
    gas_tank_size = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=100),
        missing=None,
    )
    starting_sm_o2 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=100),
        missing=None,
    )
    starting_thb = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=9, max=18),
        missing=None,
    )
    limiter = fields.String(
        metadata={"description": f"must be one of: {limiter_list}"},
        missing=None,
    )
    intervention = ma.auto_field(missing=None)
    performance_metric_1 = fields.String(
        metadata={"description": f"must be one of: {performance_metric_list}"},
        missing=None,
    )
    performance_metric_2 = fields.String(
        metadata={"description": f"must be one of: {performance_metric_list}"},
        missing=None,
    )
    performance_metric_1_value = fields.Integer(
        metadata={"description": "value in regards to chosen performance metric"},
        validate=validate.Range(min=0, max=1500),
        missing=None,
    )
    performance_metric_2_value = fields.Integer(
        metadata={"description": "value in regards to chosen performance metric"},
        validate=validate.Range(min=0, max=1500),
        missing=None,
    )
    ...
    def validate_vl_side(self, value):
        if value not in ["right", "left"] and value != None:
            raise ValidationError(
                f"{value} not a valid option. must be 'right' or 'left'"
            )
    ...
    def limiter_picklist(self, value):
        if not value in self.limiter_list and value != None:
            raise ValidationError(
                "limiter entry invalid. Please use one of the following:"
                f" {self.limiter_list}"
            )
    ...
    def performance_metric_1_picklist(self, value):
        if not value in self.performance_metric_list and value != None:
            raise ValidationError(
                "performance_metric_1 entry invalid. Please use one of the"
                f" following: {self.performance_metric_list}"
            )
    ...
    def performance_metric_2_picklist(self, value):
        if not value in self.performance_metric_list and value != None:
            raise ValidationError(
                "performance_metric_2 entry invalid. Please use one of the"
                f" following: {self.performance_metric_list}"
            )
...
class LungAssessmentSchema(ma.SQLAlchemySchema):
    ...
    bag_size = fields.Float(
        metadata={"description": "in liters"},
        validate=validate.Range(min=0, max=12),
        missing=None,
    )
    duration = fields.Integer(
        metadata={"description": "in seconds"},
        validate=validate.Range(min=0, max=400),
        missing=None,
    )
    breaths_per_minute = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=120),
        missing=None,
    )
    max_minute_volume = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=600),
        missing=None,
    )
    liters_min_kg = fields.Float(
        metadata={"description": "liters per minute per kg"},
        validate=validate.Range(min=0, max=110),
        missing=None,
    )
...
class MoxyRipExaminationSchema(Schema):
    ...
    avg_power = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=1800),
        missing=None,
    )
    hr_max_min = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=220),
        missing=None,
    )
...
class MoxyTries(Schema):
    one = fields.Nested(
        MoxyRipExaminationSchema, missing=MoxyRipExaminationSchema().load({})
    )
    two = fields.Nested(
        MoxyRipExaminationSchema, missing=MoxyRipExaminationSchema().load({})
    )
...
class MoxyRipSchema(Schema):
    ...
    smo2_tank_size = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=110),
        missing=None,
    )
    thb_tank_size = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=9, max=18),
        missing=None,
    )
    performance_baseline_smo2 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=110),
        missing=None,
    )
    performance_baseline_thb = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=9, max=18),
        missing=None,
    )
    recovery_baseline_smo2 = fields.Integer(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=110),
        missing=None,
    )
    recovery_baseline_thb = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=9, max=18),
        missing=None,
    )
    avg_watt_kg = fields.Float(
        metadata={"description": ""},
        validate=validate.Range(min=0, max=60),
        missing=None,
    )
    avg_interval_time = fields.Integer(
        metadata={"description": "seconds"},
        validate=validate.Range(min=0, max=400),
        missing=None,
    )
    avg_recovery_time = fields.Integer(
        metadata={"description": "seconds"},
        validate=validate.Range(min=0, max=400),
        missing=None,
    )
    ...
    limiter = fields.String(
        metadata={
            "description": (f"must be one of the following choices: {limiter_options}")
        },
        missing=None,
    )
    ...
    def valid_limiter(self, value):
        if value not in self.limiter_options and value != None:
            raise ValidationError(
                f"{value} is not a valid limiter option. Use one of the"
                f" following {self.limiter_options}"
            )
...
class FitnessQuestionnaireSchema(ma.SQLAlchemyAutoSchema):
    ...
    user_id = fields.Integer(missing=0)
    timestamp = fields.DateTime(
        metadata={"description": "timestamp of questionnaire. Filled by backend"}
    )
    physical_goals = fields.List(
        fields.String,
        metadata={
            "description": (
                "List of sources of stress. Limit of three from these"
                f" options: {physical_goals_list}. If other, must specify"
            )
        },
        missing=[None],
    )
    current_fitness_level = fields.Integer(
        metadata={"description": "current fitness level 1-10"},
        validate=validate.Range(min=1, max=10),
        missing=None,
    )
    goal_fitness_level = fields.Integer(
        metadata={"description": "goal fitness level 1-10"},
        validate=validate.Range(min=1, max=10),
        missing=None,
    )
    trainer_expectations = fields.List(
        fields.String,
        metadata={
            "description": (
                "Client's expectation for their trainer. Choice of:"
                f" {trainer_goals_list}"
            )
        },
        missing=[None],
    )
    sleep_hours = fields.String(
        metadata={
            "description": (
                "nightly hours of sleep bucketized by the following options:"
                f" {sleep_hours_options_list}"
            )
        },
        missing=None,
    )
    sleep_quality_level = fields.Integer(
        metadata={"description": "current sleep quality rating 1-10"},
        validate=validate.Range(min=1, max=10),
        missing=None,
    )
    stress_level = fields.Integer(
        metadata={"description": "current stress rating 1-10"},
        validate=validate.Range(min=1, max=10),
        missing=None,
    )
    stress_sources = fields.List(
        fields.String,
        metadata={
    ...
    lifestyle_goals = fields.List(
        fields.String,
        metadata={
            "description": (
                "List of lifestyle change goals. Limit of three from these"
                f" options: {lifestyle_goals_list}. If other, must specify"
            )
        },
        missing=[None],
    )
    ...
    energy_level = fields.Integer(
        validate=validate.Range(min=1, max=10),
        missing=None,
        metadata={"description": "current energy rating 1-10"},
    )
    libido_level = fields.Integer(
        metadata={"description": "current libido rating 1-10"},
        validate=validate.Range(min=1, max=10),
        missing=None,
    )
    ...
    def validate_stress_sources(self, value):
        for item in value:
            if item not in self.stressors_list and item != None:
                raise ValidationError(
                    f"{item} not a valid option. must be in" f" {self.stressors_list}"
                )
    ...
    def validate_lifestyle_goals(self, value):
        for item in value:
            if item not in self.lifestyle_goals_list and item != None:
                raise ValidationError(
                    f"{item} not a valid option. must be in"
                    f" {self.lifestyle_goals_list}"
                )
        if len(value) > 3:
    ...
    def validate_sleep_hours(self, value):
        if value not in self.sleep_hours_options_list and value != None:
            raise ValidationError(
                f"{value} not a valid option. Must be one of"
                f" {self.sleep_hours_options_list}"
            )
