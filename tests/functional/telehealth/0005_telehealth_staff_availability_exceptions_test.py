...
def test_post_staff_availability_exception(test_client):
    telehealth_exceptions_post_data["bad_data_1"][0]["exception_date"] = str(
        current_date
    )
    ...
    telehealth_exceptions_post_data["bad_data_2"][0]["exception_date"] = str(
        current_date + relativedelta(months=+6, days=+1)
    )
    ...
    telehealth_exceptions_post_data["good_data"][0]["exception_date"] = str(
        current_date
    )
    ...
    assert (
        response.json["exceptions"][0]["exception_start_time"]
        == telehealth_exceptions_post_data["good_data"][0]["exception_start_time"]
    )
    assert (
        response.json["exceptions"][0]["exception_end_time"]
        == telehealth_exceptions_post_data["good_data"][0]["exception_end_time"]
    )
    assert (
        response.json["exceptions"][0]["is_busy"]
        == telehealth_exceptions_post_data["good_data"][0]["is_busy"]
    )
    assert (
        response.json["exceptions"][0]["label"]
        == telehealth_exceptions_post_data["good_data"][0]["label"]
    )
...
def test_get_staff_availability_exception(test_client):
    ...
    assert (
        response.json[0]["exception_start_time"]
        == telehealth_exceptions_post_data["good_data"][0]["exception_start_time"]
    )
    assert (
        response.json[0]["exception_end_time"]
        == telehealth_exceptions_post_data["good_data"][0]["exception_end_time"]
    )
    assert (
        response.json[0]["is_busy"]
        == telehealth_exceptions_post_data["good_data"][0]["is_busy"]
    )
    assert (
...
def test_celery_delete_exceptions_task(test_client):
    ...
    exception = TelehealthStaffAvailabilityExceptions.query.filter_by(
