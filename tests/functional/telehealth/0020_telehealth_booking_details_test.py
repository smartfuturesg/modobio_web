...
def test_post_booking_details(test_client):
    #using first booking that comes up in query from those created in tests 1-4
    booking = test_client.db.session.query(TelehealthBookings).first()
    ...
    booking =test_client.db.session.query(TelehealthBookings).all()[-1]
...
def test_put_booking_details(test_client):
    #using first booking that comes up in query from those created in tests 1-4
    booking =test_client.db.session.query(TelehealthBookings).first()
    #The index in TelehealthBookings is the booking_id
    booking_id = booking.idx
...
def test_get_booking_details(test_client):
    #using first booking that comes up in query from those created in tests 1-4
    booking =test_client.db.session.query(TelehealthBookings).first()
    #The index in TelehealthBookings is the booking_id
    booking_id = booking.idx
...
def test_delete_booking_details(test_client):
    booking = test_client.db.session.query(TelehealthBookings).first()
    booking_id = booking.idx
