...
def add_blood_glucose_data(test_client):
    ...
    """
    test_client.mongo.db.wearables.insert_many(
        [blood_glucose_data_1, blood_glucose_data_2]
    )
...
def add_blood_pressure_data(test_client):
    ...
    """
    test_client.mongo.db.wearables.insert_many(
        [blood_pressure_data_1, blood_pressure_data_2]
    )
...
@pytest.fixture(scope="function")
def add_blood_pressure_data_multiple_sources(test_client):
    """
    Add mock wearable data for blood pressure calculation tests
    """

    blood_pressure_data_1["wearable"] = "MANUAL"
    ...
    del_query = {"user_id": test_client.client_id}
    test_client.mongo.db.wearables.delete_many(del_query)
...
def add_cgm_data(test_client):
    ...
    for i, dat in enumerate(csv_data):
        if (
            i > 0
            and parse(dat["timestamp"]).time() < csv_data[i - 1]["timestamp"].time()
        ):
                    ...
                    "data": {
                        "body": {
                            "glucose_data": {
                                ...
                                "day_avg_blood_glucose_mg_per_dL": cgm_sum
                                / len(cgm_samples),
                            }
                        }
                    },
...
def fitbit_data(test_client):
    ...
    """
    test_client.mongo.db.wearables.insert_many(
        [wearables_fitbit_data_1, wearables_fitbit_data_2]
    )
...
def bp_data_fixture(test_client):
    ...
    test_client.mongo.db.wearables.insert_many(
        [test_8100_data_past_week, test_8100_data_week_to_month_ago]
    )
...
def bp_30_days_data(test_client):
    ...
    for i, dat in enumerate(csv_data):
        if (
            ...
            hr_samples = [
                {"timestamp": parse(dat["timestamp"]), "bpm": int(dat["bpm"])}
            ]
            bp_samples = [
                {
