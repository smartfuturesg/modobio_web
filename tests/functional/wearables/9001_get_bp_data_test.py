...
def test_get_bp_data(test_client, add_blood_pressure_data):
    ...
    response = test_client.get(
        f"/v2/wearables/data/blood-pressure/{test_client.client_id}",
...
def test_get_bp_data_with_dates(test_client, add_blood_pressure_data):
    ...
    response = test_client.get(
        f"/v2/wearables/data/blood-pressure/{test_client.client_id}?start_date={start_date.isoformat()}&end_date={end_date.isoformat()}",
        headers=test_client.provider_auth_header,
        content_type="application/json",
    )
