...
from .data import clients_policies_data, clients_consult_data, clients_subscription_data
...
def test_post_subscription_contract(test_client):
    ...
    time.sleep(6)
    client_subscription = (
        ClientSubscriptionContract.query.filter_by(user_id=test_client.client_id)
        .order_by(ClientSubscriptionContract.signdate.desc())
        .first()
    )

    assert response.status_code == 201
    assert (
        client_subscription.signdate.strftime("%Y-%m-%d")
        == clients_subscription_data["signdate"]
    )
...
def test_post_consult_contract(test_client):
    ...
    time.sleep(3)
    client_consult = (
        ClientConsultContract.query.filter_by(user_id=test_client.client_id)
        .order_by(ClientConsultContract.signdate.desc())
        .first()
    )

    assert response.status_code == 201
    assert (
        client_consult.signdate.strftime("%Y-%m-%d") == clients_consult_data["signdate"]
    )
...
def test_post_policies_contract(test_client):
    ...
    time.sleep(3)
    client_policies = (
        ClientPolicies.query.filter_by(user_id=test_client.client_id)
        .order_by(ClientPolicies.signdate.desc())
        .first()
    )

    assert response.status_code == 201
    assert (
        client_policies.signdate.strftime("%Y-%m-%d")
        == clients_policies_data["signdate"]
    )
