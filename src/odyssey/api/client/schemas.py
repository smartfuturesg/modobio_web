...
class ClientSearchItemsSchema(Schema):
    ...
    modobio_id = fields.String(
        required=False, validate=validate.Length(min=0, max=12), missing=None
    )
...
class ClinicalCareTeamAuthorizationNestedSchema(Schema):
    ...
    clinical_care_team_authorization = fields.Nested(
        ClinicalCareTeamAuthorizaitonSchema(many=True), missing=[]
    )
