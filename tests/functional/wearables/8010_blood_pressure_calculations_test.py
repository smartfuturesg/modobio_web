...
def test_blood_pressure_calculations_default_date_filters(test_client, add_blood_pressure_data):
    ...
    # All of our data will live in block_seven and block_three because the hour of the timestamp in the mock data is always changed to these
    ...
    # Check block_seven data
    assert response.json.get('block_seven').get('average_systolic') == 77
    assert response.json.get('block_seven').get('average_diastolic') == 137
    assert response.json.get('block_seven').get('average_pulse') == 122
    assert response.json.get('block_seven').get('max_systolic') == 85
    assert response.json.get('block_seven').get('max_diastolic') == 140
    assert response.json.get('block_seven').get('total_pulse_readings') == 2
    assert response.json.get('block_seven').get('total_bp_readings') == 2
    assert response.json.get('block_seven').get('min_diastolic') == 135
    assert response.json.get('block_seven').get('min_systolic') == 70
...
def test_blood_pressure_calculations_start_date_param(test_client, add_blood_pressure_data):
...
def test_blood_pressure_calculations_end_date_param(test_client, add_blood_pressure_data):
...
def test_blood_pressure_calculations_start_and_end_date_params(test_client, add_blood_pressure_data):
