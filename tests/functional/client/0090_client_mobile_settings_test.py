...
def test_post_client_mobile_settings(test_client):
    ...
    settings = (
        test_client.db.session.query(ClientMobileSettings)
        .filter_by(user_id=test_client.client_id)
        .one_or_none()
    )

    notifications = (
        test_client.db.session.query(ClientNotificationSettings)
        .filter_by(user_id=test_client.client_id)
        .all()
    )
...
def test_get_client_mobile_settings(test_client):
    ...
    assert (
        response.json["general_settings"]["date_format"]
        == clients_mobile_settings["general_settings"]["date_format"]
    )
    assert (
        response.json["notification_type_ids"]
