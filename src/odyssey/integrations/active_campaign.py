...
from odyssey.api.user.models import (
    User, UserActiveCampaign, UserActiveCampaignTags, UserSubscriptions
)
...
class ActiveCampaign:
    ...
    def check_contact_existence(self, user_id) -> bool:
        ...
        if data['meta']['total'] == '1':
            ...
            if not ac_contact:
                ...
                ac_contact = UserActiveCampaign(user_id=user_id, active_campaign_id=contact_id)
    ...
    def update_ac_contact_info(self, user_id, first_name=None, last_name=None, email=None):
        ...
        logger.info('Update Active Campaign contact information error:'
                    f' {response.text}')
        return response
    ...
    def convert_prospect(self, user_id, ac_contact_id):
        ...
        if (
            user.is_client and converted_tags_dict['Converted - Client'] not in user_tags.keys()
        ...
        if (
            user.is_provider and converted_tags_dict['Converted - Provider'] not in user_tags.keys()
        ):
    ...
    def get_user_tags(self, user_id=None, ac_contact_id=None) -> dict:
        ...
        if not ac_contact_id:
            ac_contact = UserActiveCampaign.query.filter_by(user_id=user_id).one_or_none()
            if not ac_contact:
                logger.error('No Active Campaign contact ID found with provided'
                             ' user ID.')
                raise ValueError('No Active Campaign contact ID found with provided'
                                 ' user ID.')
