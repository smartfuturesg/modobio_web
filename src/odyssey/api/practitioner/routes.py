...
from odyssey.api.practitioner.schemas import (
    PractitionerConsultationRateInputSchema, PractitionerCredentialsInputSchema,
    PractitionerCredentialsSchema, PractitionerDeleteCredentialsSchema,
...
class PractionerCredentialsEndpoint(BaseResource):
    ...
    def get(self, user_id):
        ...
        query = (
            db.session.execute(
                select(PractitionerCredentials).where(PractitionerCredentials.user_id == user_id)
            ).scalars().all()
        )
        return {'items': query}
    ...
    def put(self, user_id):
        ...
        payload.pop(
            'staff_role'
        )  # Staff_role passed in schema but isn't part of PractitionerCredentials model

        curr_credentials = PractitionerCredentials.query.filter_by(
...
class PractitionerConsultationRates(BaseResource):
    ...
    def get(self, user_id):
        ...
        """
        staff_user_roles = (
            db.session.query(StaffRoles).filter(StaffRoles.user_id == user_id).all()
        )
...
class PractitionerOrganizationAffiliationAPI(BaseResource):
    ...
    @responds(
        schema=PractitionerOrganizationAffiliationSchema(many=True),
        status_code=200,
        api=ns,
    )
    def get(self, user_id):
    ...
    def post(self, user_id):
        ...
        if data.organization_idx in [
            ...
            raise BadRequest(
                'Practitioner is already affiliated with organization'
...
class PractitionerTelehealthActivationEndpoint(BaseResource):
    ...
    @token_auth.login_required(user_type=('staff_self', ))
    @responds(
        schema=TelehealthStaffSettingsSchema(only=['provider_telehealth_access']),
        status_code=200,
    )
    def get(self, user_id):
    ...
    def put(self, user_id):
        ...
        telehealth_settings = TelehealthStaffSettings.query.filter_by(user_id=user.user_id
                                                                     ).one_or_none()
        if not telehealth_settings:
            # Create telehealth settings with default values and provided flag
            telehealth_settings = TelehealthStaffSettings(
                user_id=user.user_id,
                provider_telehealth_access=telehalth_access_flag,
            )
