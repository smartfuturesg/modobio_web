...
def test_post_strength_assessment(test_client, init_database):
    ...
    WHEN the '/trainer/initialquestionnaire/<client id>' resource  is requested (POST)
    ...
    response = test_client.post('/trainer/initialquestionnaire/1/',
                                headers=headers, 
                                data=dumps(payload), 
...
def test_get_strength_assessment(test_client, init_database):
    ...
    WHEN the  '/trainer/initialquestionnaire/<client id>' resource  is requested (GET)
    THEN check the response is valid
    """
    # get staff authorization to view client data
    staff = Staff().query.first()
    ...
    # send get request for client info on clientid = 1 
    response = test_client.get('/trainer/initialquestionnaire/1/',
                                headers=headers, 
                                content_type='application/json')
