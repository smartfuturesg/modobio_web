...
def test_post_subscription_grant_unverified_user(test_client, unverified_user):
    ...
    EmailVerification().complete_email_verification(
        user_id=user.user_id, code=email_verification["code"]
    )
...
def test_post_subscription_grant_non_user(test_client):
    ...
    response = test_client.post(
        "/user/client/", data=dumps(data), content_type="application/json"
    )
    ...
    # verify user email
    email_verification = UserPendingEmailVerifications.query.filter_by(
        user_id=user_id
    ).first()
    EmailVerification().complete_email_verification(
        user_id=user_id, code=email_verification.code
    )
