...
from odyssey.api.client_services.schemas import (
    CSAccountBlockSchema, CSAccountUnblockSchema, CSUserLoginSchema, CSUserSchema
)
...
class CSAccountBlockEndpoint(BaseResource):
    ...
    def post(self, user_id):
        ...
        """
        user_login = (
            db.session.execute(select(UserLogin).filter_by(user_id=user_id)).scalars().one_or_none()
        )
        ...
        cs_id = (
            f'{cs_user.firstname} {cs_user.lastname} '
        ...
        if request.json['staff']:
            ...
            logger.audit(
                f'Staff account for user {user_id} blocked '
            ...
            logger.audit(
                f'Client account for user {user_id} blocked '
                f'by client services member {cs_id}. Reason: ' + request.json['reason']
            )
    ...
    def delete(self, user_id):
        ...
        if request.json['staff']:
            ...
            logger.audit(
                f'Staff account block for user {user_id} '
            ...
            logger.audit(
                f'Client account block for user {user_id} '
                f'removed by client services member {cs_id}.'
            )
