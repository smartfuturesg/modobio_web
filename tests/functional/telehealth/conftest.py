    ...
    for i in range(10):
        ...
        creds.append(
            ProviderCredentials(
                ...
                status="Verified",
                role_id=StaffRoles.query.filter_by(
                    user_id=staff.user_id, role="medical_doctor"
                )
                .one_or_none()
                .idx,
            )
...
def staff_credentials(test_client):
    ...
    role = StaffRoles.query.filter_by(
        user_id=test_client.provider_id, role="medical_doctor"
    ).one_or_none()
...
def staff_territory(test_client):
    ...
    territory = StaffOperationalTerritories(
        user_id=test_client.provider_id, role_id=role.idx, operational_territory_id=1
    )
...
def register_device(test_client):
    ...
    device.arn = pn.register_device(
        "abc123", "debug", device_info="debug device for telehealth testing"
    )
...
def booking(test_client, payment_method):
    ...
    target_datetime = target_datetime.replace(
        ...
        if (start_minute == 60 and target_datetime.hour < 23)
        else target_datetime.hour,
    ...
    if not TelehealthStaffSettings.query.filter_by(
        user_id=test_client.provider_id
    ).one_or_none():
    ...
    chat_room = (
        test_client.db.session.execute(
            select(TelehealthChatRooms).where(TelehealthChatRooms.booking_id == bk.idx)
        )
        .scalars()
        .one_or_none()
    )
...
def staff_availabilities(test_client, telehealth_staff):
    ...
    # delete all entries
    test_client.db.session.query(TelehealthStaffAvailability).filter(
        TelehealthStaffAvailability.user_id.in_(staff_ids)
    ).delete()
    test_client.db.session.query(TelehealthStaffSettings).filter(
        TelehealthStaffSettings.user_id.in_(staff_ids)
    ).delete()
...
def upcoming_bookings(test_client, payment_method):
    ...
    for interval in intervals:
        target_datetime = datetime.now(timezone.utc) + timedelta(
            hours=interval[0], minutes=interval[1]
        )
        start_minute = target_datetime.minute + (10 - target_datetime.minute % 10)
        target_datetime = target_datetime.replace(
            hour=target_datetime.hour + 1
        ...
        chat_room = (
            test_client.db.session.execute(
                select(TelehealthChatRooms).where(
                    TelehealthChatRooms.booking_id == bk.idx
                )
            )
            .scalars()
            .one_or_none()
