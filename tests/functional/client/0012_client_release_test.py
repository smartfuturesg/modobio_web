...
def test_post_client_release(test_client):
    ...
    response = test_client.post(
        f"/client/release/{test_client.client_id}/",
    ...
    assert (
        response.json["release_to"][0]["email"]
        == clients_release_data["release_to"][0]["email"]
    )
...
def test_get_client_release(test_client):
    ...
    assert (
        response.json["release_of_other"]
        == "Only release my prescription drugs, not anything else."
    )
