...
from tests.functional.client_services.data import (
    client_services_register_user_client,
    client_services_register_user_staff)

def test_new_client_user(test_client):
    # register the new client user
    response = test_client.post(
        '/client-services/user/new/', 
    ...
    response = test_client.put(
        f'/user/registration-portal/verify?portal_id={portal_id}', 
        headers=test_client.staff_auth_header)
    ...
    response = test_client.get(
        f'/client/{new_remote_user_id}/', 
        headers=test_client.staff_auth_header,
        content_type='application/json')

    assert response.status_code == 200

def test_new_staff_user(test_client):
    # register the new client user
    response = test_client.post(
        '/client-services/user/new/', 
        headers=test_client.staff_auth_header,
