...
def test_twilio_wrapper(test_client, booking_function_scope):
    ...
    media_sid = twilio.upload_media(
        media_path="tests/functional/telehealth/test_img_weirdmole.jpg"
    )
...
def test_telehealth_transcript_get(test_client, booking_function_scope):
    ...
    # add test image file to twilio
    image_media_sid = twilio.upload_media(
        media_path="tests/functional/telehealth/test_img_weirdmole.jpg"
    )
    pdf_media_sid = twilio.upload_media(
        media_path="tests/functional/twilio/test_pdf.pdf",
    ...
    assert (
        response.json["transcript"][2]["media"][0]["content_type"] == "application/pdf"
    )
