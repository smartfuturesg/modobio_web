...
class ShoulderRotationSchema(Schema):
    ir = fields.Integer(
        metadata={"description": "internal rotation"},
        validate=validate.Range(min=0, max=110),
        missing=None,
    )
    er = fields.Integer(
        metadata={"description": "external rotation"},
        validate=validate.Range(min=0, max=130),
        missing=None,
    )
    abd = fields.Integer(
        metadata={"description": "abduction"},
        validate=validate.Range(min=0, max=90),
        missing=None,
    )
    add = fields.Integer(
        metadata={"description": "adduction"},
...
class HipRotationSchema(Schema):
    ir = fields.Integer(
        metadata={"description": "internal rotation"},
        validate=validate.Range(min=-20, max=100),
        missing=None,
    )
    er = fields.Integer(
        metadata={"description": "external rotation"},
...
class ChessBoardShoulderSchema(Schema):
    left = fields.Nested(
        ShoulderRotationSchema, missing=ShoulderRotationSchema().load({})
    )
    right = fields.Nested(
        ShoulderRotationSchema, missing=ShoulderRotationSchema().load({})
    )
...
class ChessboardSchema(Schema):
    ...
    isa_movement_list = [
        "Dynamic",
        "Static",
        "R Static/Left Dynamic",
        "L Static/Right Dynamic",
    ]

    user_id = fields.Integer(missing=0)
    timestamp = fields.DateTime()
    isa_structure = fields.String(
        metadata={"description": f"must be one of {isa_structure_list}"},
        missing=None,
    )
    isa_movement = fields.String(
        metadata={"description": f"must be one of {isa_movement_list}"},
        missing=None,
    )
    co2_tolerance = fields.Integer(
        validate=validate.Range(min=0, max=120), missing=None
    )
    shoulder = fields.Nested(
        ChessBoardShoulderSchema, missing=ChessBoardShoulderSchema().load({})
    )
    hip = fields.Nested(ChessBoardHipSchema, missing=ChessBoardHipSchema().load({}))
    notes = fields.String(
        metadata={"description": "some notes regarding this assessment"},
        missing=None,
    )
    ...
    def isa_structure_picklist(self, value):
        if not value in self.isa_structure_list and value is not None:
            raise ValidationError(
                "isa_structure entry invalid. Please use one of the"
                f" following: {self.isa_structure_list}"
            )
    ...
    def isa_movement_picklist(self, value):
        if not value in self.isa_movement_list and value is not None:
            raise ValidationError(
                "isa_movement entry invalid. Please use one of the following:"
                f" {self.isa_movement_list}"
            )
    ...
    def ravel(self, data, **kwargs):
        ...
        shoulder_l = {
            ...
            if "left_shoulder" in k
        }
        shoulder_r = {
            k.split("_")[-1]: v
            for k, v in data.__dict__.items()
            if "right_shoulder" in k
        }
        hip_l = {
            k.split("_")[-1]: v for k, v in data.__dict__.items() if "left_hip" in k
        }
        hip_r = {
            k.split("_")[-1]: v for k, v in data.__dict__.items() if "right_hip" in k
        }
        nested = {
            "user_id": data.user_id,
            "notes": data.notes,
            "isa_structure": data.isa_structure,
            "isa_movement": data.isa_movement,
            "shoulder": {"right": shoulder_r, "left": shoulder_l},
            "hip": {"right": hip_r, "left": hip_l},
        }
        return nested
