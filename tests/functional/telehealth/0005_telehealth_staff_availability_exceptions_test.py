...
from .data import telehealth_exceptions_post_data, telehealth_exceptions_post_data_bad
...
def test_post_staff_availability_exception(test_client):
    response = test_client.post(
        ...
        data=dumps(telehealth_exceptions_post_data_bad),
    ...
    assert response.json[0]['exception_booking_window_id_start_time'] == 120
    assert response.json[0]['exception_booking_window_id_end_time'] == 150
    assert response.json[0]['exception_date'] == "2030-01-01"
