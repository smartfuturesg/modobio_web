...
class OperationalTerritories(BaseResource):
    ...
    def post(self, user_id):
        ...
        current_role_ids = (
            db.session.query(StaffRoles.idx).filter(StaffRoles.user_id == user_id).all()
        )
...
class RecentClients(BaseResource):
    ...
    def get(self):
        ...
        return StaffRecentClients.query.filter_by(
            ...
            for event in query_set:
                if event.recurring:
                    ...
                    if (
                        ...
                        and event.end_date < datetime(year, month, day).date()
                    ):
                        new_query.discard(event)
                            ...
                            for _month in range(1, 13):
                                mylist.append(
                                    datetime(
                                        year,
                                        _month,
                                        calendar.monthrange(year, _month)[1],
                                    )
                                )
                        else:
