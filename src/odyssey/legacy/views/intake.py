...
def consent():
    ...
    cc = (
        ClientConsent.query.filter_by(clientid=clientid)
        .order_by(ClientConsent.revision.desc())
        .first()
    )
...
def release():
    ...
    ci = ClientInfo.query.filter_by(clientid=clientid).one()
    cr = (
        ClientRelease.query.filter_by(clientid=clientid)
        .order_by(ClientRelease.revision.desc())
        .first()
    )
...
def policies():
    ...
    ci = ClientInfo.query.filter_by(clientid=clientid).one()
    cp = (
        ClientPolicies.query.filter_by(clientid=clientid)
        .order_by(ClientPolicies.revision.desc())
        .first()
    )
...
def consult():
    ...
    ci = ClientInfo.query.filter_by(clientid=clientid).one()
    cc = (
        ClientConsultContract.query.filter_by(clientid=clientid)
        .order_by(ClientConsultContract.revision.desc())
        .first()
    )
...
def subscription():
    ...
    ci = ClientInfo.query.filter_by(clientid=clientid).one()
    cs = (
        ClientSubscriptionContract.query.filter_by(clientid=clientid)
        .order_by(ClientSubscriptionContract.revision.desc())
        .first()
    )
...
def individual():
    ...
    ci = ClientInfo.query.filter_by(clientid=clientid).one()
    cs = (
        ClientIndividualContract.query.filter_by(clientid=clientid)
        .order_by(ClientIndividualContract.revision.desc())
        .first()
    )

    form = ClientIndividualContractForm(
        obj=cs, fullname=ci.fullname, guardianname=ci.guardianname
    )
