...
def test_post_1_client_staff_bookings(
    test_client, staff_availabilities, telehealth_staff
):
    ...
    queue_data = {
        ...
        "target_date": datetime.strptime(
            telehealth_client_staff_bookings_post_1_data.get("target_date"), "%Y-%m-%d"
        ).isoformat(),
        "priority": False,
        "medical_gender": "np",
        "location_id": 1,
    ...
    conversation = TelehealthChatRooms.query.filter_by(
    ...
    staff_events = test_client.db.session.execute(
        select(StaffCalendarEvents).where(
            StaffCalendarEvents.location == f"Telehealth_{conversation.booking_id}"
        )
    ).one_or_none()[0]

    assert (
        staff_events.start_date
        == datetime.strptime(
            telehealth_client_staff_bookings_post_1_data["target_date"], "%Y-%m-%d"
        ).date()
    )
...
@pytest.mark.skip(reason="Telehealth on the Shelf")
def test_post_2_client_staff_bookings(
    test_client, telehealth_staff, staff_availabilities
):
    # add client to queue first
    queue_data = {
        "profession_type": "medical_doctor",
        "target_date": datetime.strptime(
            telehealth_client_staff_bookings_post_2_data.get("target_date"), "%Y-%m-%d"
        ).isoformat(),
        "priority": False,
        "medical_gender": "np",
        "location_id": 1,
...
@pytest.mark.skip(reason="Telehealth on the Shelf")
def test_post_3_client_staff_bookings(
    test_client, telehealth_staff, staff_availabilities
):
...
def test_get_1_staff_bookings(test_client, telehealth_staff):
    ...
    response = test_client.get(
        f"/telehealth/bookings/" f"?staff_user_id={telehealth_staff[0].user_id}",
...
def test_get_2_client_bookings(test_client):
    response = test_client.get(
        f"/telehealth/bookings/" f"?client_user_id={test_client.client_id}",
        headers=test_client.client_auth_header,
        content_type="application/json",
    )
...
@pytest.mark.skip(reason="Telehealth on the Shelf")
def test_post_4_client_staff_bookings(
    test_client, staff_availabilities, telehealth_staff
):
    ...
    staff_role = StaffRoles.query.filter_by(
        user_id=telehealth_staff[0].user_id, role="medical_doctor"
    ).one_or_none()
...
def test_get_4_staff_client_bookings(test_client, booking):
    ...
    assert (
        response_booking["status_history"][0]["reporter_role"]
        == booking.status_history[0].reporter_role
    )
