...
class ActiveCampaign:
    ...
    def request(self, method, endpoint, payload=None):
        ...
        try:
            response = requests.request(
                method, url, json=payload, headers=self.request_header
            )
    ...
    def get_list_id(self, ac_list):
        ...
        try:
            ...
            raise BadRequest(
                f"Active Campaign unable to connect. Error: {response.text}"
            )
    ...
    def check_contact_existence(self, user_id) -> bool:
        ...
        if data["meta"]["total"] == "1":
            ...
            ac_contact = UserActiveCampaign.query.filter_by(
                user_id=user_id
            ).one_or_none()
            ...
            if not ac_contact:
                logger.info(
                    "Creating UserActiveCampaign db entry for already"
                    f" existing user {email}."
                )
                ac_contact = UserActiveCampaign(
                    user_id=user_id, active_campaign_id=contact_id
                )
            ...
            response = self.request(
                "GET", endpoint=f"contacts/{contact_id}/contactLists"
            )
            ...
            if len(data["contactLists"]) > 0:
                for x in data["contactLists"]:
                    if (
                        x["list"] == self.list_id
                    ):  # Contact is already created and in list
            ...
            if not in_list:
                ...
                response = self.request(
                    "POST", endpoint=f"contactLists", payload=payload
                )
            return True
        else:
    ...
    def add_tag(self, user_id, tag_name):
        ...
        # Add tag to contact
        payload = {"contactTag": {"contact": ac_id.active_campaign_id, "tag": tag_id}}
        ...
        tag = UserActiveCampaignTags.query.filter_by(
            user_id=user_id, tag_name=tag_name
        ).one_or_none()
        if not tag:
            tag = UserActiveCampaignTags(
                user_id=user_id, tag_id=tag_id, tag_name=tag_name
            )
            db.session.add(tag)
            db.session.commit()

            logger.info(
                f"Added Active Campaign tag: {tag_name} to user with user_id:"
                f" {user_id}"
            )
        return response
    ...
    def remove_tag(self, user_id, tag_name):
        ...
        logger.info(
            f"Removed Active Campaign tag: {tag_name} from user with user_id:"
            f" {user_id}"
        )
        return response

    def update_ac_contact_info(
        self, user_id, first_name=None, last_name=None, email=None
    ):
        # Get active campaign contact id
        ac_id = UserActiveCampaign.query.filter_by(user_id=user_id).one_or_none()
        if not ac_id:
        ...
        response = self.request(
            "PUT",
            endpoint=f"contacts/{ac_id.active_campaign_id}",
            payload=payload,
        )

        logger.info(
            "Update Active Campaign contact information status code:"
            f" {response.status_code}"
        )
        logger.info(
            "Update Active Campaign contact information error:" f" {response.text}"
        )
        return response
    ...
    def delete_contact(self, user_id):
        ...
        # Delete contact info
        response = self.request(
            "DELETE", endpoint=f"contacts/{ac_id.active_campaign_id}"
        )
    ...
    def add_user_subscription_type(self, user_id):
        ...
        subscription_tags = UserActiveCampaignTags.query.filter(
            UserActiveCampaignTags.user_id == user_id,
            UserActiveCampaignTags.tag_name.contains("Subscription"),
        ).all()
        ...
        if client_sub:
            if client_sub.subscription_status == "unsubscribed":  # Unsubscribed
                ...
                client_sub.subscription_type_id == 2
            ):  # Monthly Subscription ID from LookupTable
                return self.add_tag(user_id, "Subscription - Month")
            elif (
                client_sub.subscription_type_id == 3
            ):  # Yearly Subscrioption ID from LookupTable
    ...
    def convert_prospect(self, user_id, ac_contact_id):
        ...
        if (
            user.is_client
            and converted_tags_dict["Converted - Client"] not in user_tags.keys()
            and prospect_tags_dict["Prospect - Provider"] not in user_tags.keys()
        ):
        ...
        if (
            user.is_provider
            and converted_tags_dict["Converted - Provider"] not in user_tags.keys()
        ):
    ...
    def get_user_tags(self, user_id=None, ac_contact_id=None) -> dict:
        ...
        if not ac_contact_id:
            ...
            if not ac_contact:
                logger.error(
                    "No Active Campaign contact ID found with provided" " user ID."
                )
                raise ValueError(
                    "No Active Campaign contact ID found with provided" " user ID."
                )
