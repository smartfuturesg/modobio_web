...
def test_post_power_assessment(test_client):
    ...
    assert (
        response.json["lower_watts_per_kg"]
        == trainer_power_assessment_data["lower_watts_per_kg"]
    )
    assert (
        response.json["leg_press"]["bilateral"]["attempt_1"]
        == trainer_power_assessment_data["leg_press"]["bilateral"]["attempt_1"]
    )
