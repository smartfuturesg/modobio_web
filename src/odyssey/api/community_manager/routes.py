...
from odyssey.api.community_manager.schemas import (
    PostSubscriptionGrantSchema, ProviderLicensingSchema, ProviderLiscensingAllSchema,
    ProviderRoleRequestsAllSchema, ProviderRoleRequestUpdateSchema, SubscriptionGrantsAllSchema,
...
class CMSubscriptionGrantingEndpoint(BaseResource):
    ...
    def get(self):
        ...
        subscription_grants = (
            CommunityManagerSubscriptionGrants.query.filter_by(user_id=user.user_id).all()
        )
    ...
    def post(self):
        ...
        for modobio_id in data['modobio_ids']:
            subscription_user = User.query.filter_by(modobio_id=modobio_id).one_or_none()
            if not subscription_user:
                raise BadRequest('User with modobio_id {} does not exist'.format(modobio_id))
            else:
        ...
        for user_id in user_ids:
            if current_app.testing:
                update_client_subscription(user_id, auto_renew=False)
            else:
...
class CMProviderLicensingEndpoint(BaseResource):
    ...
    def get(self):
        ...
        query = db.session.query(ProviderCredentials,
                                 User).join(User, User.user_id == ProviderCredentials.user_id)
        ...
        if start_date and end_date:
            ...
            query = query.filter(ProviderCredentials.created_at.between(start_date, end_date))
...
class CMVerifyCredentialsEndpoint(BaseResource):
    ...
    def put(self):
        ...
        query = (
            db.session.query(ProviderCredentials,
                             User).join(User, User.user_id == ProviderCredentials.user_id).filter(
...
class CMProviderRoleRequestsEndpoint(BaseResource):
    ...
    def get(self):
        ...
        query = (
            db.session.query(ProviderRoleRequests,
                             User).join(User,
                                        User.user_id == ProviderRoleRequests.user_id).order_by(
                                            ProviderRoleRequests.created_at.asc()
                                        )
        )
        if status:
        ...
        for role_request, user in items:
            ...
            if not user_current_roles.get(user.user_id):
                user_current_roles[user.user_id] = StaffRoles.query.filter_by(user_id=user.user_id
                                                                             ).all()
