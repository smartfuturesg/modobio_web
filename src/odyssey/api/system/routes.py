...
class SystemTelehealthSettingsApi(BaseResource):
    ...
    def get(self):
        costs = db.session.execute(
            select(SystemTelehealthSessionCosts, LookupCurrencies).join(
                LookupCurrencies,
                LookupCurrencies.idx == SystemTelehealthSessionCosts.currency_id,
            )
        ).all()
        ...
        session_duration = int(
            SystemVariables.query.filter_by(var_name="Session Duration")
            .one_or_none()
            .var_value
        )
        booking_notice_window = int(
            SystemVariables.query.filter_by(var_name="Booking Notice Window")
            .one_or_none()
            .var_value
        )
        confirmation_window = float(
            SystemVariables.query.filter_by(var_name="Confirmation Window")
            .one_or_none()
            .var_value
        )
        res = {
    ...
    def put(self):
        ...
        for cost in request.parsed_obj["costs"]:
            ...
            exists = SystemTelehealthSessionCosts.query.filter_by(
                profession_type=cost.profession_type,
        ...
        if "session_duration" in request.parsed_obj:
            ses_dur = SystemVariables.query.filter_by(
                var_name="Session Duration"
            ).one_or_none()
        ...
        if "booking_notice_window" in request.parsed_obj:
            book_window = SystemVariables.query.filter_by(
                var_name="Booking Notice Window"
            ).one_or_none()
            book_window.update(
                {"var_value": str(request.parsed_obj["booking_notice_window"])}
            )
            res["booking_notice_window"] = book_window.var_value
        if "confirmation_window" in request.parsed_obj:
            con_window = SystemVariables.query.filter_by(
                var_name="Confirmation Window"
            ).one_or_none()
            con_window.update(
                {"var_value": str(request.parsed_obj["confirmation_window"])}
            )
...
class SystemDeleteUserApi(BaseResource):
    ...
    @ns.doc(
        params={
            "delete_type": (
                "Denotes what portion of the user should be deleted. Can be"
                " 'client','staff', or 'both "
            )
        }
    )
    ...
    def delete(self, user_id):
        ...
        return {
            "message": (
                f"User with id {user_id} has been removed with a delete_type"
                f" of {delete_type}."
            )
        }
