...
class PowerAssessmentSchema(Schema):
    ...
    def ravel(self, data, **kwargs):
        ...
        recent_physical = (
            MedicalPhysicalExam.query.filter_by(user_id=data.user_id).order_by(
...
class MoxyRipSchema(Schema):
    ...
    limiter = fields.String(
        metadata={'description': (f'must be one of the following choices: {limiter_options}')},
        missing=None,
    )
