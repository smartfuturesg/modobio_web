...
def test_create_tag(test_client, create_client_contact):
    ...
    ac_tag = UserActiveCampaignTags.query.filter_by(
        user_id=test_client.client_id, tag_name="Persona - Client"
    ).one_or_none()
...
def test_add_user_subscription_tag(test_client, create_client_contact):
    ...
    assert response.status_code == 201
    ac_tag = UserActiveCampaignTags.query.filter_by(
        user_id=test_client.client_id, tag_name="Subscription - Annual"
    ).one_or_none()
    assert ac_tag.tag_id == int(data["contactTag"]["id"])
...
def test_convert_client_prospect(test_client, create_client_contact):
    ...
    ac_tag = UserActiveCampaignTags.query.filter_by(
        user_id=test_client.client_id, tag_name="Converted - Client"
    ).one_or_none()
...
def test_convert_provider_prospect(test_client, create_provider_contact):
    ...
    ac_tag = UserActiveCampaignTags.query.filter_by(
        user_id=test_client.provider_id, tag_name="Converted - Provider"
    ).one_or_none()
