...
def test_get_telehealth_activation(test_client):
    ...
    assert (
        response.json["provider_telehealth_access"]
        == staff_settings.provider_telehealth_access
    )
...
def test_update_telehealth_activation(test_client):
    ...
    assert response.status_code == 200
    assert (
        response.json["provider_telehealth_access"] != previous_telehealth_access_flag
    )
