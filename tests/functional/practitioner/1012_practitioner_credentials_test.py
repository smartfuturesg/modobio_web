...
def test_post_1_credentials(test_client):
    ...
    credentials = (
        ProviderCredentials.query.filter_by(user_id=test_client.staff_id)
        .order_by(ProviderCredentials.idx.desc())
        .first()
    )
...
def test_post_2_credentials_bad_payload_with_state_missing_credential_number(
    test_client,
):
    response = test_client.post(
        f"/practitioner/credentials/{test_client.staff_id}/",
