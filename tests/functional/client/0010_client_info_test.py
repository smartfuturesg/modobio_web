...
def test_put_client_info(test_client):
    ...
    assert (
        response.json["client_info"]["primary_macro_goal"]
        == "('Gain the best understanding I can of my health at any point in time.',)"
    )
    assert (
        response.json["client_info"]["primary_goal_description"]
        == "To try out something new"
    )
    assert response.json["client_info"]["territory_id"] == 1
...
def test_creating_new_client(test_client):
    ...
    user = User.query.filter_by(
        email=users_new_user_client_data["user_info"]["email"]
    ).first()
    assert response.status_code == 201
    assert user.email == users_new_user_client_data["user_info"]["email"]
    assert (
        response.json["user_info"]["modobio_id"] == None
    )  # email not verified yet, so no modobio_id
    ...
    valid_credentials = base64.b64encode(
        f"{users_new_user_client_data['user_info']['email']}:{password}".encode("utf-8")
    ).decode("utf-8")
