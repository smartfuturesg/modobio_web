...
ns_dev = Namespace(
    "postman",
    description=("[DEV ONLY] Endpoint to download a Postman collection of the API."),
)
...
class PostmanEndpoint(BaseResource):
    ...
    def get(self):
        ...
        """
        data = json.dumps(
            ns_dev.apis[0].as_postman(urlvars=True, swagger=True), indent=2
        )

        response = Response(data, mimetype="application/json", direct_passthrough=True)
