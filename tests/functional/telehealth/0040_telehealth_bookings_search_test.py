...
def test_get_booking_by_date_time(test_client, booking_function_scope):
    ...
    booking_dates = [
        booking["target_date_utc"] for booking in response.json.get("bookings")
    ]
    assert response.status_code == 200
    assert all(
        [
            booking_date == str(booking_function_scope.target_date_utc)
...
def test_get_booking_by_status(test_client, booking_function_scope):
    ...
    booking_statuss = [
        booking["status_history"][0]["status"]
        for booking in response.json.get("bookings")
    ]
    assert response.status_code == 200
...
def test_get_booking_by_order(test_client, booking_function_scope):
    ...
    #
    chat_room = TelehealthChatRooms.query.filter_by(
        booking_id=booking_function_scope.idx
    ).first()

    new_target_date = booking_function_scope.target_date_utc + timedelta(days=1)

    new_booking = TelehealthBookings(
        **{
            k: v
            for k, v in booking_function_scope.__dict__.items()
            if k not in ("_sa_instance_state", "created_at", "updated_at", "idx")
        }
    )

    new_chatroom = TelehealthChatRooms(
        **{
            k: v
            for k, v in chat_room.__dict__.items()
            if k not in ("_sa_instance_state", "created_at", "updated_at", "room_id")
        }
    )
